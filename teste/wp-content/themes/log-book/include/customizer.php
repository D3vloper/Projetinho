...
function log_book_customize_register( $wp_customize ) {
   ...
   $wp_customize->add_control(
	...
	$wp_customize->add_setting( 'log_book_primary_color',
	    array(
	        'default'           => $default['log_book_primary_color'],
	...
	$wp_customize->add_control(
	    new WP_Customize_Color_Control(                 
	        ...
	        array(
	            'label'       => esc_html__( 'Primary Color', 'log-book' ),
	            'description' => esc_html__( 'Change your whole site color from here. More are available in premium version.', 'log-book' ),
	            'section'     => 'colors',  
	        )
	    )
	);
 ...
 * @return Boolean
 */
function log_book_sanitize_rgba( $color ) {
...
if ( ! function_exists( 'log_book_default_theme_options' ) ) :
	...
	function log_book_default_theme_options() {
		...
		//Primary color
		$defaults['log_book_primary_color']    = '#E73E3B';

		//overlay ccolor
		$defaults['log_book_overlay_bg_color']= 'rgba(231,62,59, 0.7)';
